...
const textAnmition = () => {
    ...
    const typingSpeed = 200; // 每毫秒打印的字符数
    const deleteSpeed = 30; // 每毫秒删除的字符数
    const pauseDuration = 1500; // 打印完毕后暂停的时间（毫秒）
    
    const animationEl = document.getElementById("typing-animation");
    const textEl = document.getElementById("text");
    const textArr = ['遇见你是命运的安排，爱上你是我情非得已','遇见你之前，我的世界是黑白的；遇见你之后，我的世界变成了彩虹','不知道什么时候，你出现在我的生命里，但我知道，遇见你是我这一生最美丽的意外','相逢一笑泯恩仇，此后心情更落花']
    let text = textArr[2]
    let currentIndex = 0;
    ...
    function animate() {
      ...
      if (!isDeleting && currentText === text) {
        ...
        isDeleting = false;
          currentIndex = 0;
          text = textArr[ Math.floor(Math.random() *4)]
